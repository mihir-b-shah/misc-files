...
TRAINING_EP = 0.00001  # epsilon-greedy parameter for training
...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if(np.random.uniform() < epsilon):
        return (np.random.randint(0, NUM_ACTIONS), np.random.randint(0, NUM_OBJECTS))
    else:
        v = np.argmax(q_func[state_1, state_2])
        return (v//NUM_OBJECTS, v%NUM_OBJECTS)
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    """
    # TODO Your code here
    q_func[current_state_1, current_state_2, action_index, object_index] *= 1-ALPHA
    q_func[current_state_1, current_state_2, action_index, object_index] += ALPHA*reward
    if(terminal == False):
        q_func[current_state_1, current_state_2, action_index, object_index] += ALPHA*GAMMA*np.max(q_func[next_state_1, next_state_2])         
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if for_training:
            # update Q-function.
            # TODO Your code here
            tabular_q_learning(q_func, current_room_index, current_quest_index, action_index, object_index, reward, next_room_index, next_quest_index, terminal)
            pass
