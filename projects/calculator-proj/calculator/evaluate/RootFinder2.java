...
public class RootFinder2 {  
    ...
    public static void manageDeletion(ArrayList<Double> parts, double operator, int i, double x) {
        if (operator == 30.0) {
            parts.set(i, hash(evalK(parts.get(i - 1), x) + evalK(parts.get(i + 1), x)));
        } else if (operator == 31.0) {
            parts.set(i, hash(evalK(parts.get(i - 1), x) - evalK(parts.get(i + 1), x)));
        } else if (operator == 32.0) {
            parts.set(i, hash(evalK(parts.get(i - 1), x) * evalK(parts.get(i + 1), x)));
        } else if (operator == 33.0) {
            parts.set(i, hash(evalK(parts.get(i - 1), x) / evalK(parts.get(i + 1), x)));
        }
